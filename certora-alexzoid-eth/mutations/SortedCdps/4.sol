...
contract SortedCdps is ISortedCdps {
    ...
    function getCdpCountOf(
        address owner,
        bytes32 startNodeId,
        uint maxNodes // @audit uint256 and uint mix
...
@@ -505,7 +505,7 @@ contract SortedCdps is ISortedCdps {
         // List must contain the node
         require(contains(_id), "SortedCdps: List does not contain the id");
         // NICR must be non-zero
-        require(_newNICR > 0, "SortedCdps: NICR must be positive");
+        require(_newNICR > 0, "SortedCdps: NICR must be positive"); // @audit the same check in _insert()
...
**************************** Diff Block End *****************************/
    ...
    function reInsert(
        ...
        _insert(_id, _newNICR, _prevId, _nextId); // @audit return id like in insert
    }
